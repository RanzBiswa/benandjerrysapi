...
func TestIsPositiveInt(t *testing.T) {
	...
	var tests = []struct {
		...
		{"-2147483648", false},          //Signed 32 Bit Min Int
		{"2147483647", true},            //Signed 32 Bit Max Int
		{"-2147483649", false},          //Signed 32 Bit Min Int - 1
		{"2147483648", true},            //Signed 32 Bit Max Int + 1
		{"4294967295", true},            //Unsigned 32 Bit Max Int
		{"4294967296", true},            //Unsigned 32 Bit Max Int + 1
		{"-9223372036854775808", false}, //Signed 64 Bit Min Int
		{"9223372036854775807", true},   //Signed 64 Bit Max Int
	...
	for _, test := range tests {
		actual := IsPositiveInt(test.param)
		if actual != test.expected {
			t.Errorf("Expected IsInt(%q) to be %v, got %v", test.param, test.expected, actual)
		}
	}
...
func TestIsPositiveIntIncludingZero(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		actual := IsPositiveIntIncludingZero(test.param)
		if actual != test.expected {
			t.Errorf("Expected IsInt(%q) to be %v, got %v", test.param, test.expected, actual)
		}
	}
